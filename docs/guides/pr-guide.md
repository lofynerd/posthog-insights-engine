...
For the PR Telegram group. Focused on brand visibility, audience growth, media reach, engagement, referral sources, sentiment, and content performance — no AWS or infrastructure detail.
...
Each sends a real PostHog chart (whichever metric moved most that period, e.g. audience growth, engagement) plus a short, reasoned AI caption — not a wall of text. Reports are cached per day, so re-running the same command twice in a day is instant.
